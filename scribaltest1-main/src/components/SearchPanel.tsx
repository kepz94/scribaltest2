...
export default function SearchPanel(props: SearchPanelProps) {
  ...
  // Which result row is showing its in-place verse context (3 verses each side).
  ...
  // Verses grouped by chapter scope ("Alma 5"), kept in scripture order, so a
  // result can show a few verses of surrounding context inline. index is
  // already in order, so each chapter's list is verse-ordered.
  const chapterMap = useMemo(() => {
    const m: Record<string, IndexEntry[]> = {};
    index.forEach((e) => {
      const scope = e.reference.replace(/:\d+$/, "");
      (m[scope] = m[scope] || []).push(e);
    });
    return m;
  }, [index]);

  // The matched verse plus up to 3 verses on each side, within its chapter.
  const contextFor = (reference: string) => {
    const scope = reference.replace(/:\d+$/, "");
    const arr = chapterMap[scope];
    if (!arr) return [];
    const i = arr.findIndex((e) => e.reference === reference);
    if (i < 0) return [];
    return arr.slice(Math.max(0, i - 3), i + 4).map((e) => ({
      reference: e.reference,
      verse: e.verse,
      text: e.text,
      isMatch: e.reference === reference,
    }));
  };
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const key = r.reference + "|" + i;
                        setPreviewKey((p) => (p === key ? null : key));
                      }}
                      title="Show a few verses of context here"
                      style={{
                        padding: "3px 10px",
                        borderRadius: "999px",
                        border: "1px solid var(--border)",
                        background:
                          previewKey === r.reference + "|" + i
                            ? "var(--soft)"
                            : "transparent",
                        color: "var(--muted)",
                        cursor: "pointer",
                    ...
                    >
                      {previewKey === r.reference + "|" + i ? "Hide" : "Preview"}
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
